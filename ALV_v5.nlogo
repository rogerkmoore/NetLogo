globals [ Xhome Yhome X0 Y0 X1 Y1 X2 Y2 XALVhome YALVhome XALVnow YALVnow GoHome]
...
to setup
  ...
  ;; set up three turtles
  create-turtles 5 [
    set size 40
  ]

  ;; configure each turtle differently
  ask turtle 0 [ set shape "house" set color orange set xcor -100 + random 20 set ycor 100 + random 10 ]
  ask turtle 1 [ set shape "tree" set color green set xcor 100 + random 20 set ycor 100 + random 10 ]
  ask turtle 2 [ set shape "car" set color red set xcor -10 + random 20 set ycor -100 + random 10 ]
  ask turtle 3 [ set shape "ufo top" set color black ]
  ask turtle 4 [ set shape "ufo top" set color blue ]

  ;; set up one turtle as a robot and place it near the centre
  ask turtle 4 [
    set xcor random 50 - 25
    set ycor random 50 - 25
  ]

  ;; set up links between the robot and each turtle
  ask turtle 4 [
    create-link-with turtle 0 [ set color white ]
    create-link-with turtle 1 [ set color white ]
    create-link-with turtle 2 [ set color white ]
  ]
...
to go
  ask turtle 4 [ FollowMouse ]
  if GoHome [ ask turtle 4 [ move ] ]
  tick
end

;; allow the robot to be moved manually using the mouse and compute ALVnow
to FollowMouse
  if mouse-down? [ set xcor mouse-xcor set ycor mouse-ycor ]
  ask link 4 0 [ set X0 sin(link-heading) set Y0 cos(link-heading)]
  ask link 4 1 [ set X1 sin(link-heading) set Y1 cos(link-heading)]
  ask link 4 2 [ set X2 sin(link-heading) set Y2 cos(link-heading)]
  set XALVnow (X0 + X1 + X2) / 3
  set YALVnow (Y0 + Y1 + Y2) / 3
end

; capture the robot's current location and compute the ALV
to StoreHome
  ask turtle 4 [
    set Xhome xcor
    set Yhome ycor
  ...
  ask link 4 1 [ set X1 sin(link-heading) set Y1 cos(link-heading)]
  ask link 4 2 [ set X2 sin(link-heading) set Y2 cos(link-heading)]
  set XALVhome (X0 + X1 + X2) / 3
  set YALVhome (Y0 + Y1 + Y2) / 3
end
...
;; compute the Home Vector and move in that direction until home reached
to move
  if xcor < Xhome + 1 AND xcor > Xhome - 1 AND ycor < Yhome + 1 AND ycor > Yhome - 1 [ set GoHome false ]
  if GoHome [
    set heading 180 + atan (XALVnow - XALVhome) (YALVnow - YALVhome)
    forward 1
  ]
end
...
This model demonstrates the principle of using the 'Average Landmark Vector' (ALV) to return home.

## HOW IT WORKS

There are three landmarks, and one robot.

## HOW TO USE IT

Click the SETUP button to create the robot and landmarks.  Click GO to start the simulation.  The robot can be moved with the mouse.  Set a suitable home location by pressing the SET HOME button.  Move the robot to another position and press the RETURN HOME button.
...
* Moore, R. K. (2025).  NetLogo Average Landmark Vector model.  School of Computer Science, University of Sheffield, UK.
